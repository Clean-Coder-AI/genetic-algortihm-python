...
class SimpleGeneticAlgorithm:
    ...
    def run_algorithm(self):
        ...
        max_generations = 10000  # Maximum number of generations allowed
        generations = 0

        while generations < max_generations and my_pop.get_fittest().get_fitness() < len(self.solution):
            print(f"Generation: {generations} Correct genes found: {my_pop.get_fittest().get_fitness()} Genes: {my_pop.get_fittest()}")

            my_pop = self.evolve_population(my_pop)
            generations += 1

        if my_pop.get_fittest().get_fitness() == len(self.solution):
